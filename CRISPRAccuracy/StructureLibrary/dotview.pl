...
OPTIONS
        ...
        -zeros	Remove all entries with zero probabilities to get a cleaner plot
...
my ($i_help, $i_man, $i_dot, $i_out, $i_seq, $i_reg, $i_col, $i_crop, $i_str, 
		$i_str_db, $i_ne_str, $i_strict, $i_pdf, $i_png, $i_replace, $i_zeros);
...
if($i_dot =~ /(.+)\.ps/){
	...
	if($i_replace){
		system("rm '$i_dot'") unless ($i_dot eq $i_out);
	}
} else {
	...
	open(IN_HANDLE, "<$i_dot") || die "couldn't open the following file: $i_dot/n";
		...
		while ($line = <IN_HANDLE>){
			...
			# get the base pairs, crop and colour them according to the given parameters
				...
				# remove zero probabilities if parameter given
				if($i_zeros){
					if($prob == 0.00){
						next;
					}
